#!/usr/bin/env python3
"""
Phase 6 Task 6.1.3: Model Comparison Investigation (Simplified)

This module conducts comprehensive comparison of core models after the data leakage fix,
focusing on Linear, Ridge, Lasso, RandomForest, and GradientBoosting models.
...
class ModelComparisonInvestigator:
    """
    Conducts comprehensive comparison and analysis of core models
    ...
    def get_model_configurations(self) -> Dict[str, Any]:
        """Get core model configurations for comparison."""
        models = {
            'LinearRegression': {
    ...
    def evaluate_model_performance(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        """Comprehensive evaluation of core models."""
        ...
        for model_name, model_config in models.items():
            ...
            try:
                ...
                feature_importance = self.get_feature_importance(model, X_train, y_train, model_name)
    ...
    def get_feature_importance(self, model, X: pd.DataFrame, y: pd.Series, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Method 3: Permutation importance (for all models) - simplified
            logger.info(f"Computing permutation importance for {model_name}...")
            perm_importance = permutation_importance(
                model, X.iloc[:min(500, len(X))], y.iloc[:min(500, len(y))], 
    ...
    def generate_learning_curves(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        """Generate learning curves for core models."""
        ...
        train_sizes = np.linspace(0.1, 1.0, 8)  # Reduced for speed
        ...
        for model_name, model_config in models.items():
            ...
            try:
                ...
                # Generate learning curve with reduced CV folds for speed
    ...
    def plot_feature_importance_comparison(self, performance_results: Dict[str, Any]):
        ...
        try:
            ...
            # Get top 10 features from first model
            first_model = list(interpretable_models.values())[0]
            features = first_model['features']
            importances = first_model['importances_mean']
            ...
            for i, (model_name, importance_data) in enumerate(interpretable_models.items()):
                model_features = importance_data['features']
                model_importances = importance_data['importances_mean']
                
                # Align importances with top features
                aligned_importances = []
                for feature in top_features:
                    if feature in model_features:
                        idx = model_features.index(feature)
                        aligned_importances.append(model_importances[idx])
                    else:
            ...
            ax.set_title('Top 10 Feature Importance Comparison (Interpretable Models)')
    ...
    def run_comprehensive_comparison(self) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison_results = {
                ...
                'analysis_type': 'comprehensive_model_comparison_simplified',
                'dataset_info': {
                    'shape': list(df.shape),
